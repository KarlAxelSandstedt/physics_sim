...
GLubyte * (*mglGetString)(GLenum) = NULL;
...
#define	LOAD_PROC(mgl_fp, type, gl_name)								\
	...
	if ((mgl_fp) == NULL) 										\
	{												\
		fprintf(stderr, "System could not load %s\n", (gl_name));	\
	}
...
void win_mglSwapBuffers(struct window *win)
{
	SwapBuffers(win->device_context);
}

static void win_mgl_functions_init(void)
{
	...
	mglSwapBuffers = &win_mglSwapBuffers;
}

void win_mgl_init()
{
	win_mgl_functions_init();
